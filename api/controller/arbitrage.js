...
async function getTopTranscations() {
  ...
  try {
    ...
    // );
    const recentTransactions = await client.query(
      `SELECT * FROM  arbitrages_view av WHERE av.profit_usd < 2000000000000000000 AND av.transaction_hash ORDER BY av.block_number DESC LIMIT 2;`
    );
    // const sandwichedTransaction = await client.query(
    ...
    // );
    const sandwichedTransaction = await client.query(
      `SELECT * FROM sandwiched_view sv WHERE sv.profit_usd < 2000000000000000000 AND sv.frontrun_transaction_hash ORDER BY sv.block_number DESC LIMIT 2;`
    );
    arbtxs = recentTransactions.rows.map((d) => {
      let obj = {};
      obj.created_at = d.created_at;
